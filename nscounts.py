...
report = wikipedia.Page(site, 'Wikipedia:Database reports/Page count by namespace')
report.put(report_template % (current_of, '\n'.join(output), ns_count_tcol, ns_count_r_tcol, ns_count_gtotal), 'updated page', True, False)
cursor.close()
conn.close()
